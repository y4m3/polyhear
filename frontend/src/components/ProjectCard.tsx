...
export function ProjectCard({ project, onClick, isWorktree = false }: ProjectCardProps) {
  const hasError = !!project.error
  const hasFailed = project.build.status === 'fail' || project.test.status === 'fail'
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1" title="Test">
                ...
                {project.test.failed > 0 && (
                  <span className="text-xs text-red-400">({project.test.failed})</span>
                )}
              </div>
            </div>
